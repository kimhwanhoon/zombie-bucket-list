...
const SignupForm = () => {
  ...
  const handleSignUpButtonClick = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try{
      ...
      await supabase.from('users').insert({nickname, email, password, signupDate, profileImage: newProfileImg});
  ...
  return (
    ...
    <form>
      ...
      <input type="nickname" value={nickname} onChange={onChange} name="nickname" placeholder="닉네임"/>
      <input type="email" value={email} onChange={onChange} name="email" placeholder="이메일"/>
      <input type="password" value={password} onChange={onChange} name="password" placeholder="비밀번호"/>
      <input type="password" value={checkPassword} onChange={onChange} name="checkPassword" placeholder="비밀번호 체크"/>
      <button type="submit" onClick={handleSignUpButtonClick}>회원가입</button>
    </form>
